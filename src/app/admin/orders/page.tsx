...
import { Search, X, Eye, RefreshCw, Package, Clock, Truck, CheckCircle2, XCircle } from "lucide-react";
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="font-heading text-3xl font-extrabold text-ink">Orders</h1>
          <p className="text-muted mt-1">{orders.length} total orders</p>
        </div>
        <button onClick={fetchOrders}
          className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-ink hover:bg-slate-50 transition-colors">
                  ...
                  <tr key={o.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => setShowDetail(o)}
                        className="p-2 hover:bg-royal/5 rounded-lg text-navy transition-colors" title="View Details">
                        <Eye size={16} />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowDetail(null)}>
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-100">
              <div>
                <h2 className="font-heading text-xl font-bold text-ink">Order Details</h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 rounded-xl p-3">
                  ...
                  <select
                    ...
                    className={`text-xs font-semibold px-2.5 py-1 rounded-lg border-0 outline-none cursor-pointer mt-1 ${STATUS_STYLES[showDetail.status] || "bg-slate-100"}`}
                  >
                    {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s.replace("_", " ")}</option>)}
                  </select>
                </div>
