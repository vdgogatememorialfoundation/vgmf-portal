...
function CategoryFilter({ categories, active }: { categories: string[]; active: string }) {
  return (
    <div className="flex flex-wrap justify-center gap-2">
      <Link href="/articles" className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${!active || active === "All" ? "bg-navy text-white shadow-md shadow-navy/20" : "bg-white border border-gray-200 text-ink-soft hover:border-navy/30 hover:text-navy"}`}>
        All
      </Link>
      {categories.map(cat => (
        <Link key={cat} href={`/articles?category=${encodeURIComponent(cat)}`} className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${active === cat ? "bg-navy text-white shadow-md shadow-navy/20" : "bg-white border border-gray-200 text-ink-soft hover:border-navy/30 hover:text-navy"}`}>
          {cat}
        </Link>
      ))}
    </div>
      ...
      <section className="relative bg-navy py-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 backdrop-blur-sm rounded-full border border-white/10 mb-6">
            ...
            <span className="text-gold-light text-xs font-bold tracking-widest uppercase">Knowledge Base</span>
          </span>
          <h1 className="font-heading text-5xl md:text-6xl font-extrabold text-white mb-4">
            Articles & <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold to-gold-light">Research</span>
          </h1>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredArticles.map(article => (
                <Link key={article.id} href={`/articles/${article.slug}`} className="group bg-white rounded-3xl border border-gray-100 p-7 card-hover block">
                  {article.category && (
                    <span className="inline-block px-3 py-1 bg-navy/5 text-navy text-xs font-bold rounded-full mb-4 uppercase tracking-wider">
                      {article.category}
                    </span>
                  )}
                  <h3 className="font-heading text-lg font-extrabold text-navy mb-3 group-hover:text-gold transition-colors leading-snug line-clamp-2">
                    {article.title}
                  </h3>
                  {article.excerpt && (
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                    ...
                    <ArrowRight size={16} className="text-navy/30 group-hover:text-gold group-hover:translate-x-1 transition-all" />
                  </div>
                </Link>
              ))}
            </div>
