      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-navy rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Stethoscope className="text-gold" size={32} />
          </div>
          <h1 className="font-heading text-3xl font-extrabold text-navy">Doctor Login</h1>
          <p className="text-muted mt-2">VGMF Doctor Portal</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border p-8 space-y-5">
          ...
          <div>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
              className="w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-navy/20" placeholder="doctor@vaidyagogate.org" />
          </div>
          <div>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? "Verifying..." : "Sign In as Doctor"}
          </button>
        </form>
